...
- (void)racMemoryLeakTestSample2 {
    ...
    //    帖子上说 subscriber 中 nextBlock 持有 singals 和当前 signal（即 subject），并且当前 signal（即 subject 持有了 subsciber），所有出现了环引用
    //    但是看了最新的源码，发现已经没有了 signals 的局部变量，并且 subscriber 中的 nextBlock 不持有当前 signal（即 subject），所以只是  当前 signal（即 subject 持有了 subscriber），所以没有了环引用
